...
def jd_login(driver, wait):
    ...
    if len(login_btn) == 0:
        ...
        time.sleep(1)
        # 等待用户扫码操作（最多30秒）
        WebDriverWait(driver, 80).until(EC.presence_of_element_located((By.CSS_SELECTOR, '#key')))


def sent_keyword(driver, keyword):
    """ 打开网页并输入关键词 """
    driver.get('https://www.jd.hk/')
    time.sleep(1)
    # 关键词输入
    ...
    # 模拟点击按销量排行
    sales_btn = driver.find_element(By.CSS_SELECTOR,
                                    '.filter_filter__WyDpV > div.topFilter_f-line__C2RLR.topFilter_top__Ly58e > div.topFilter_f-sort__iJE5o > button:nth-child(2)')
    sales_btn.click()
    time.sleep(1)
...
def jd_get_dir_info(driver, save_path_dir, store_num, keyword):
    ...
    #                                     f".productList_goods-list__wMdJe.productList_item-style-v3__7xJxO > ul > li:nth-last-child({1 + x*2}) > div:nth-child(2) > div > div.productList_p-img__FGbqQ > a > div > img")
    #
    #     # 使用ActionChains模拟鼠标悬停操作
    #     ActionChains(driver).context_click(element_to_hover_over).perform()
    #     time.sleep(1)
    # time.sleep(2)
    # 商品标题和链接
    for l in driver.find_elements(By.CSS_SELECTOR,
                                  '#root > div:nth-child(6) > div.search_g-main1__ZCpBn > div > div > div.productList_goods-list__wMdJe > ul > li:nth-child(n) > div:nth-child(2) > div > div.productList_p-name__eCbI6 > a'):
        links.append(l.get_attribute('href'))
        titles.append(l.text.strip().replace('京东国际', ''))

    # 主图链接
    for m in driver.find_elements(By.CSS_SELECTOR,
                                  '#root > div:nth-child(6) > div.search_g-main1__ZCpBn > div > div > div.productList_goods-list__wMdJe > ul > li:nth-child(n) > div:nth-child(2) > div > div.productList_p-img__FGbqQ > a > div > img'):
        main_pic_link = m.get_attribute('src')
        main_pic_links.append(main_pic_link)

    # 店铺名称
    for s in driver.find_elements(By.CSS_SELECTOR,
                                  '#root > div:nth-child(6) > div.search_g-main1__ZCpBn > div > div > div.productList_goods-list__wMdJe > ul > li:nth-child(n) > div:nth-child(2) > div > div.productList_p-shop-name__-JOwm > p > a:nth-child(1)'):
        stores.append(s.text.strip())

    # 评论数
    for e in driver.find_elements(By.CSS_SELECTOR,
                                  '#root > div:nth-child(6) > div.search_g-main1__ZCpBn > div > div > div.productList_goods-list__wMdJe > ul > li:nth-child(n) > div:nth-child(2) > div > div.productList_p-buy__DEak9 > div > a > span > b'):
        evaluate_nums.append(e.text.strip())

    # 类型(自营或非自营)
    for t in driver.find_elements(By.CSS_SELECTOR,
                                  '#root > div:nth-child(6) > div.search_g-main1__ZCpBn > div > div > div.productList_goods-list__wMdJe > ul > li:nth-child(n) > div:nth-child(2) > div > div.productList_p-icons__gFUWq'):
...
def jd_get_detail_info(url, title, driver, all_url):
    ...
    if not sku_id_list:
        ...
        sku_name_list.append(driver.find_element(By.CSS_SELECTOR, '.itemInfo-wrap > div.sku-name').text.strip())
    ...
    # print(main_pic_list)
    print(len(sku_name_list), len(sku_id_list), len(product_link_list), len(QR_pic_link_list), len(main_pic_list),
          len(all_url))
    # print(all_url)
...
def jd_get_all_data(dir_data, driver, all_url, keyword):
    ...
    for url, title, store, evaluate_num in zip(url_list, title_list, store_list, evaluate_num_list):
        sku_name_list, sku_id_list, product_link_list, QR_pic_link_list, main_pic_list, place, all_url = jd_get_detail_info(
            url, title, driver, all_url)
        half_data = pd.DataFrame({
            '二维码链接': QR_pic_link_list,
            # [0]] + ['\\'] * (len(sku_id_list) - 1) if QR_pic_link_list else QR_pic_link_list,
            '主图链接': main_pic_list,
            '关键词': keyword,
...
def spilt_price(result_data):
    """ 已弃用 """
    prices = result_data['价格'].tolist()
    # 分割价格数据并找到最大单元格长度
    max_length = 0
    parsed_prices = []
    for price in prices:
        parsed_price = str(price).split(' ')
        max_length = max(max_length, len(parsed_price))
    ...
    for i in range(max_length):
        ...
        for price in parsed_prices:
            if i < len(price):
                new_column.append(price[i])
            else:
                new_column.append('/')
        new_columns.append(new_column)
    # 将新列数据添加到 'result_data' 中，并删除原有的 '价格' 列
    for i, column in enumerate(new_columns):
        result_data[f'价格{i + 1}'] = column
    del result_data['价格']
    return result_data


def jd_transform_table(html_str):
    """ 京东商品信息前端展示设计（扫码填价版） """
    ...
    for idx, row in enumerate(table.find_all('tr')):
        ...
        img_src = row.select('td img')[1]['src']
        index = row.select('td')[0].text
        keyword = row.select('td')[3].text[:5]
        whole_keyword = row.select('td')[3].text
        link = row.select('td a')[0]['href']
        # type = row.select('td')[2].text
        whole_title = row.select('td')[5].text
        title = row.select('td')[5].text[:8] + '...' + row.select('td')[5].text[-8:]  # 标题过长部分隐藏
        shop = row.select('td')[6].text
        comment = row.select('td')[8].text
        item_id = row.select('td')[9].text
        whole_attribute = row.select('td')[10].text
        attribute = row.select('td')[10].text[:15] + '...'
        ...
        new_html += f'''
            <div class="grid-item">
                <div class="img-container">
                    <img src="{link_src}" height=100 width=100 alt="" title="{item_id}">
                    <a href="{link}">
                        <img src="{img_src}" height=100 width=120 alt="" title="{link}">
                    </a>
                </div>
                <div class="info">
                    <div class="item-info">
                        <div class="index">{index}</div>
                        <div class="keyword" title="{whole_keyword}">{whole_keyword}</div>
                    </div>
                    <div class="item-info">
                    ...
                    <div class="attribute" title="{whole_attribute}">{whole_attribute}</div>
                    <div class="reserved">
                        <input class="fill-input" type="text" onkeypress="nextInput(event, {idx*6+1})">
                        <input class="fill-input" type="text" onkeypress="nextInput(event, {idx*6+2})">
                        <input class="fill-input" type="text" onkeypress="nextInput(event, {idx*6+3})">
                        <input class="fill-input" type="text" onkeypress="nextInput(event, {idx*6+4})">
                        <input class="fill-input" type="text" onkeypress="nextInput(event, {idx*6+5})">
                        <input class="fill-input" type="text" onkeypress="nextInput(event, {idx*6+6})">
                    </div>
                </div>
            </div>
        '''
...
def jd_transform_table2(html_str):
    """ 京东商品信息前端展示设计（机器填价版--直接勾选） """
    # 使用 BeautifulSoup 解析 HTML
    soup = BeautifulSoup(html_str, 'html.parser')
    # 获取表格元素
    table = soup.select_one('table > tbody')
    ...
    for idx, row in enumerate(table.find_all('tr')):
        ...
        img_src = row.select('td img')[1]['src']
        index = row.select('td')[0].text
        keyword = row.select('td')[3].text[:8] + '...' + row.select('td')[3].text[-8:]
        whole_keyword = row.select('td')[3].text
        link = row.select('td a')[0]['href']
        # type = row.select('td')[2].text
        whole_title = row.select('td')[5].text
        ...
        new_html += f'''
            <div class="grid-item">
                <div class="img-container">
                    <a href="{link}">
                        <img src="{img_src}" height=100 width=130 alt="" title="{link}">
                    </a>
                    <a href="{link_src}">
                        <div class="keyword" title="{whole_keyword}">{whole_keyword}</div>
                    </a>
                ...
                <div class="info">
                    <div class="item-info">
                        <div class="index">{index}</div>
                        <div class="reserved">{is_reserved}</div>
                    </div>
                    <div class="item-info">
                        <div class="item-id">{item_id}</div>
                        <div class="comment">{comment}</div>                       
...
def jd_filter_data(white_list, black_list, all_data):
    ...
    filter_data1 = set_shop_index(filter_data)
    filter_data1['索引'] = filter_data1['索引'].apply(lambda x: int(x))
    # index_column = filter_data1.pop('索引')
    # filter_data1.insert(2, '索引', index_column)  # 将新的'索引'列移动到第一列

    return filter_data1


def jd_show_on_html(save_path_dir, filter_data, table_name):
    """ 将数据展示在html网页上 """
    ...
    with open(rf'{save_path_dir}\jd-{table_name}.html', 'w', encoding='utf-8') as file:
...
if __name__ == '__main__':
    ...
    # 筛选并保存数据
    # filter_data = jd_filter_save_data(white_list, black_list, save_path_dir, keyword, all_data2)
    all_data2.to_excel(rf'{save_path_dir}\jd-批量关键词测试.xlsx', index=False)
    # 关闭浏览器
    driver.close()
    # 生成html文件
    html_content = jd_show_on_html(save_path_dir, all_data2, table_name)
